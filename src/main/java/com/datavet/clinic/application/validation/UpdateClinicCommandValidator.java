package com.datavet.clinic.application.validation;
...
import com.datavet.shared.application.validation.Validator;
import org.springframework.stereotype.Component;
import org.springframework.util.StringUtils;

/**
 * Validator for UpdateClinicCommand using shared validation framework.
 * Value objects (Address, Email, Phone) handle their own validation.
 */
@Component
public class UpdateClinicCommandValidator implements Validator<UpdateClinicCommand> {
    
    @Override
    public ValidationResult validate(UpdateClinicCommand command) {
        ...
        // Validate subscription status (optional)
        if (command.getSuscriptionStatus() != null && command.getSuscriptionStatus().length() > 50) {
            result.addError("suscriptionStatus", "Subscription status must not exceed 50 characters");
        }
